...
$(document).ready(function () {
   ...
   $('div.unconfirmed').click(function() {
      var el = $(this);
      $.get('/single/ajax/checkin?u=' + this.id.split('_').pop(), function(data) {
         if (validateResponse(data))
         {
            var parent_id = el.parent().attr('id').replace('unconfirmed', 'confirmed');
            var parent = $('#' + parent_id);
            el.detach().appendTo(parent);
            el.removeAttr('id').removeClass('unconfirmed').addClass('confirmed').off('click').prepend('<span>' + (data.checkinID) + '</span>');
         }
      });
   });
